...
export class RoutesComponent implements OnInit, AfterViewInit {
  ...
  removeStudent(index: number) {
    ...
    const studentToDelete = {
      ...
      route_assignment_uuid:
        removedStudent.route_assignment_uuid || this.route_assignment_uuid,
    };
  ...
  updateRoute() {
    ...
    added.forEach((student, index) => {
      student.student_order = startingOrder + index;
    });
  ...
  openDetailModal(route_uuid: string) {
    axios
      ...
      .then((response) => {
        const detailData = response.data.route_assignment[0];

        this.route_name = response.data.route_name;
        this.route_description = response.data.route_description;
